...
function WelcomeContent() {
  ...
  const userIdDisplayStyle = { borderColor: `${config.theme_color}33` };
    ...
    <KillSwitch pageName="crash">
      ...
      <style jsx global>{`
          ...
          *{box-sizing:border-box}
          ...
          .display-circle{ width:clamp(300px, 80vw, 420px); height:clamp(300px, 80vw, 420px); border-radius:50%;display:flex;align-items:center;justify-content:center;position:relative; background: radial-gradient(ellipse at center, rgba(0,0,0,0.18), rgba(0,0,0,0.45)); border:1px solid ${config.theme_color}1a; overflow:hidden; }
          #crashValue{ font-size: clamp(4rem, 15vw, 6rem); font-weight:900; letter-spacing: 1px;transition:transform .18s ease, opacity .18s ease; text-align:center;white-space:nowrap; -webkit-font-smoothing:antialiased; }
          ...
          .connection-status, .user-id-display { position: fixed; top: 15px; display: flex; align-items: center; gap: 8px; font-size: 16px; color: white; z-index: 9999; background: rgba(0,0,0,0.3); padding: 5px 10px; border-radius: 8px; border: 1px solid; }
