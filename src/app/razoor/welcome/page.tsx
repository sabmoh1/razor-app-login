...
// This is the new welcome page for 'razoor'
// It does NOT connect to any WebSocket.
// It reads prediction values from localStorage, managed by the new admin page.
  ...
  useEffect(() => {
    ...
    if (canvas) {
        ...
        if (ctx) {
            ...
            (window as any).__razoorCleanup = cleanup;
        }
    }
    
    return () => {
        if ((window as any).__razoorCleanup) {
          (window as any).__razoorCleanup();
        } else {
            clearInterval(timerInterval);
        }
    };
    ...
    <>
      ...
      <style jsx global>{`
          ...
          *{box-sizing:border-box}
          ...
          .brand h1{ font-size:2rem;margin:0;color:var(--neon-primary);letter-spacing:4px;font-weight:900; text-shadow: 0 0 2px rgba(255,255,255,0.7), 0 0 5px rgba(255,255,255,0.7), 0 0 8px rgba(255,255,255,0.7); }
      ...
      <div className="wrap" onClick={handleScreenClick}>
        <div className="panel">
          <div className="brand">
            ...
            <h1 className="neon-label" style={{color: 'var(--neon-primary)', textShadow: '0 0 2px #fff, 0 0 3px #fff, 0 0 5px #fff'}}>RAZOOR</h1>
          </div>
...
export default function RazoorWelcomePage() {
  return (
